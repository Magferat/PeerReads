...
exports.createRequest = async (req, res) => {
    ...
    if (!book || book.status !== 'Available') return res.status(400).json({ message: 'Book not available' });
    if (String(book.owner) === String(req.user._id)) return res.status(400).json({ message: 'Cannot request your own book' });

    const br = await BorrowRequest.create({ book: book._id, lender: book.owner, borrower: req.user._id, message });
    await Notification.create({ user: book.owner, type: 'BorrowRequest', message: `New request for "${book.title}"` });
    res.json(br);
};
